...
export const TestBuilder: React.FC = () => {
  ...
  const updateDescribeBool = (data: any) =>
    dispatch(UpdateDescribeBoolean(data));
  ...
  useEffect(() => {
    ...
    x[`${describeIndex}`] = (
      <DescribeBlock
        ...
        id={describeIndex}
        itIndex={itIndex}
        describeProp={`${describeIndex}`}
        removeDescribe={removeDescribe}
      />
    );
  ...
  function removeDescribe(id: number): boolean {
    ...
    if (Object.keys(describesFromStore).length > 1) {
      ...
      for (const it of Object.keys(describesFromStore[`${id}`])) {
        for (const expect of Object.keys(itFromStore[`${it}`])) {
  ...
  return (
    <div className="testBuilder">
      ...
      <div className="addtestbutton">
        <button className="adddescribe" onClick={addDescribe}>
          + Describe Block
        </button>
      </div>
    </div>
