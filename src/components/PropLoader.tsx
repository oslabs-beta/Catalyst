...
export const PropLoader: React.FC<Props> = ({ id }) => {
  ...
  const [propArray, updatePropArray] = useState<object[]>([]);
  ...
  const updatePropBooleanInStore = (data: any) =>
    dispatch(UpdateDescribeBoolean(data));

  useEffect(() => {
    const display = [];
    for (const x of propArray) {
      for (const ele of Object.values(x)) {
        display.push(ele);
      }
    }
      ...
      prop[`describe${id}prop${count}`] = (
        <div className="propChild" id={`describe${id}prop${count}`} key={count}>
          ...
          <button
            ...
            onClick={test}
            id={`describe${id}prop${count}button`}
          >
            X
          </button>
        </div>
