...
export const DescribeBlock: React.FC<Props> = ({
  describeProp,
  removeDescribe,
}) => {
  ...
  const [arrayOfIt, updateItArray] = useState([]);
  ...
  function removeDescribeComponent(): void {
    removeDescribe(parseInt(describeProp, 10));
  }

  return (
    <div className="describeBlock" id={`describeBlock${describeProp}`}>
      <button className="remove" onClick={removeDescribeComponent}>
        X
      </button>
      <div className="describe">
        ...
        <input
          ...
          type="text"
          onChange={(e) => addComponentName(e.target.value)}
        ...
        <form>
          <PropLoader
            id={`${describeProp}`}
            key={`describePropLoader${describeProp}`}
          />
        </form>
        {arrayOfIt}
      </div>
      <button className="addit" onClick={addIt}>
        + It Statement
      </button>
    </div>
  );
