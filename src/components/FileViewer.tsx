...
export const FileViewer: React.FC = () => {
  ...
  const fileViewPath = useSelector((state: any) => state.fileToView);
  ...
  function pathToText() {
    const fileContent = fs.readFileSync(fileViewPath);
    const x: any = fileContent.toString();
    let counter = 0;
  ...
  return fileViewPath === "" ? (
    <></> // fragment shorthand over React.Fragment
  ) : (
    <div className="codeBlock">
      <div className="buttonHolder">
        <button onClick={clearFile} className="clearFile remove">
          X
        </button>
        <br />
      </div>
