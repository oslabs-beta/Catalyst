...
export const FileTree: React.FC = () => {
  ...
  const extensionGrabber = (ext: string) => {
    ...
    if (
      ext
        .split(".")
        [ext.split(".").length - 1].includes(
          "eslint" || ext.split(".")[1].includes("eslint")
        )
    ) {
      return "eslint";
    }
    ...
    return (
      <ul className="FileTree">
        {files.map((file: any, id: number) => {
            ...
            return (
              <li key={id}>
                <span
                  className="directory"
                  key={id}
                  onClick={() => handleToggle(file.filepath)}
                >
                  <span>{FILE_ICONS[extension]}</span>
                  {file.name}
                </span>
          ...
          return (
            <li key={id}>
              <a onClick={setFileView} id={file.filepath} key={id}>
                <span id="icon">{FILE_ICONS[extension]}</span>
                {file.name}
              </a>
  ...
  return fileTree ? traverseFileTree(fileTree) : <></>; // using fragment shorthand over React.Fragment
};
